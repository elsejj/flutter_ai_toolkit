...
class LlmMessageView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (llmFlex, spaceFlex) = flexRatio(context);
    return Row(
      children: [
        Flexible(
          flex: llmFlex,
        ...
        Flexible(flex: spaceFlex, child: SizedBox()),
      ],
    );
  }

  (int, int) flexRatio(BuildContext context) {
    final viewModel = ChatViewModelProvider.of(context);
    final chatStyle = LlmChatViewStyle.resolve(viewModel.style);
    final llmFlex = (chatStyle.llmMessageWidthPercent * 100).toInt();
    final spaceFlex = 100 - llmFlex;

    final gcd = llmFlex.gcd(spaceFlex);
    return (llmFlex ~/ gcd, spaceFlex ~/ gcd);
  }
}
